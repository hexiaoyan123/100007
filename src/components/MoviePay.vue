  ...
  <div class="container">
    ...
    <div class="ticket-cont">
      ...
      <!-- <div class="ticket-seats">
        <span class="ticket-seat" v-for="(item, index) in seats" :key="index">{{item}}</span>
      </div> -->
    ...
    <!-- <div class="ticket-btn" @click="goWechat" v-if="ticket.dataStatus == 1 && toBePaid > 0 && payChannel.indexOf(8) >= 0 && (! $route.query.wechat)" v-show="timer != '00:00'">
      获得微信授权
    </div> -->
    <!-- <div class="ticket-btn" @click="payOk" v-if="ticket.dataStatus == 1 && (toBePaid == 0 || payChannel.indexOf(8) < 0 || $route.query.wechat)" v-show="timer != '00:00'">
      确认支付
    </div> -->
    ...
    <div class="ticket-ctrl">
      ...
      <div class="ticket-ctrl-right"  @click="goWechat" v-if="ticket.dataStatus == 1 && toBePaid > 0 && payChannel.indexOf(8) >= 0 && (! $route.query.wechat)" v-show="timer != '00:00'">获得微信授权</div>
...
export default {
  ...
  methods: {
    ...
    payOk () {
      ...
      // 验证微信补差
      if(vm.payChannel.indexOf(8) >= 0 && (vm.checkPayCard.money-0) < (parseInt(vm.orderAmount-0)*100)){
      ...
      this.$http.post('/panda-cinema-api/v1/payment', postData, this.$init.channel.movie).then(function (response) {
    ...
    goOnceCard (payChannel) {
      window.localStorage.setItem('payChannel', JSON.stringify({
        id: payChannel.id,
    ...
    getOrder () {
      let vm = this;
      this.$http.get('/panda-cinema-api/v1/getOrder/' + this.$route.params.id, this.$init.channel.movie).then(function (response) {
        if(response.data.code == '1000'){
    ...
    getListPayChannel () {
      let vm = this;
      this.$http.get('/panda-cinema-api/v1/listPayChannel', this.$init.channel.movie).then(function (response) {
        if(response.data.code == '1000'){
