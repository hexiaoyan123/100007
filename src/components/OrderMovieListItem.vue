    ...
    <div class="movie-item">
      <div class="movie-item-top" @click="goOrderDetails">
        ...
        <div class="movie-item-right">
          ...
          <!-- <h4 class="unoverflow">{{info.cinemaName}} {{info.cinemaHallName}}</h4> -->
          <!-- <p class="unoverflow">总价: ¥ {{info.orderAmount | moneyFormat}}</p> -->
          <p class="unoverflow">{{info.cinemaName}} {{info.cinemaHallName}}</p>
        </div>
      </div>
      <!-- <div class="movie-item-bottom" v-if="info.dataStatus == 2">
...
export default {
  ...
  methods: {
    // 查看订单
    goOrderDetails () {
      this.$router.push('/orderMovieDetails/'+this.info.id);
    },
    // 去支付
    goPlay () {
      this.$router.push('/moviePay/'+this.info.id);
    },
    // 刷新列表
    refresh () {
    ...
    cancelOrder () {
      let vm = this;
      this.$http.get('/panda-cinema-api/wap/cancelOrder/'+this.info.id, this.$init.channel.movie).then(function (response) {
