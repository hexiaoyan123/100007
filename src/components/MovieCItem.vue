...
export default {
  ...
  methods: {
    ...
    getCounty () {
      let vm = this;
      this.$http.get('/panda-cinema-api/v1/listCounty/'+this.city.city.id, this.$init.channel.movie).then(function (response) {
    ...
    getCinema (clear) {
      ...
      this.$http.post(url + this.cinema.pageNum + '/20', postData, this.$init.channel.movie).then(function (response) {
        if(response.data.code == '1000'){
          if(response.data.result){
