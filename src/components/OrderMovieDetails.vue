<template>
  <div class="container">
    ...
    <ul class="list">
      ...
      <li class="item">
        <div class="item-left">
          <b>{{order.showTime | timeToStr}}</b>
        </div>
        <div class="item-right"></div>
      </li>
      <li class="item">
        <div class="item-left">
          <b>{{order.cinemaName}}</b> {{order.cinemaHallName}}
        </div>
        <div class="item-right"></div>
      </li>
      <li class="item">
    ...
    <!-- <ul class="list">
      <li class="item">
        <div class="item-left">
         <p>{{order.showType | showTypeToStr}} 选座票 x {{order.quantity}}张 </p>
         <p>{{order.showTime | timeToStr}} </p>
         <p>{{order.rowCol}} </p>
         <p>{{order.cinemaHallName}} </p>
        </div>
        <div class="item-right"></div>
      </li>
      <li class="item">
        <div class="item-left">
         <p>总价: ¥ {{order.orderAmount | moneyFormat}}</p>
         <p>优惠: ¥ 0</p>
         <p>实付: ¥ <span class="on">{{order.orderAmount | moneyFormat}}</span></p>
        </div>
        <div class="item-right"></div>
      </li>
    </ul> -->
...
export default {
  ...
  methods: {
    ...
    // 去支付
    goPay () {
      this.$router.push('/moviePay/'+this.order.id);
    },
    // 删除订单
    delOrder () {
      let vm = this;
    ...
    cancelOrder () {
      let vm = this;
      this.$http.get('/panda-cinema-api/v1/cancelOrder/'+ this.order.id, this.$init.channel.movie).then(function (response) {
        if(response.data.code == '1000'){
          vm.goBack()
        }else{
          vm.$toast(response.data.msg)
